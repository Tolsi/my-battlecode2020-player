...
public class AAPFGridGraph {
    ...
    public MapLocation findFirstBlockedTile(int x1, int y1, int dx, int dy) {
        ...
        if (dx >= dy) {
            while (true) {
                ...
                if (f >= dx) {
                    if (isBlocked(x1 + offsetX, y1 + offsetY))
                        return new MapLocation(x1+offsetX, y1+offsetY);
                    y1 += signY;
                    f -= dx;
                }
                if (f != 0 && isBlocked(x1 + offsetX, y1 + offsetY))
                    return new MapLocation(x1+offsetX, y1+offsetY);
                if (dy == 0 && isBlocked(x1 + offsetX, y1) && isBlocked(x1 + offsetX, y1 - 1))
                    return new MapLocation(x1+offsetX, -1);
            ...
            while (true) {
                ...
                if (f != 0 && isBlocked(x1 + offsetX, y1 + offsetY))
                    return new MapLocation(x1+offsetX, y1+offsetY);
                if (dx == 0 && isBlocked(x1, y1 + offsetY) && isBlocked(x1 - 1, y1 + offsetY))
                    return new MapLocation(-1, y1+offsetY);
